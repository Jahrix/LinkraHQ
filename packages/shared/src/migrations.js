import { AppStateSchema, ExportBundleSchema, SCHEMA_VERSION } from "./schema.js";
import { z } from "zod";
...
export function migrateStateToCurrent(state) {
    const now = new Date().toISOString();
    const rawProjects = Array.isArray(state?.projects) ? state.projects : [];
    const projects = rawProjects.map((project, projectIndex) => {
        ...
        const migratedTasks = tasks.map((task, taskIndex) => {
            ...
            const linkedCommit = typeof task?.linkedCommit?.sha === "string" && task.linkedCommit.sha
                ? {
                    sha: task.linkedCommit.sha,
                    shortSha: typeof task.linkedCommit.shortSha === "string" && task.linkedCommit.shortSha
        ...
        return {
            ...
            remoteRepo: typeof project?.remoteRepo === "string" && project.remoteRepo
                ? project.remoteRepo
            ...
            healthScore: typeof project?.healthScore === "number" && Number.isFinite(project.healthScore)
                ? project.healthScore
                : null,
            archivedAt,
            createdAt,
    ...
    const projectNameToId = new Map(projects
        .map((project) => [project.name, project.id])
        .filter((entry) => Boolean(entry[0] && entry[1])));
    const mapProjectRef = (projectRef) => {
        if (typeof projectRef !== "string" || !projectRef)
            return null;
        if (projectNameToId.has(projectRef))
            return projectNameToId.get(projectRef) ?? null;
        return projectRef;
    };
    const migratedRoadmap = (Array.isArray(state?.roadmapCards) ? state.roadmapCards : []).map((card, index) => {
        ...
        return {
            ...
            lane: typeof card?.lane === "string" && VALID_ROADMAP_LANES.has(card.lane)
                ? card.lane
                : "next",
    ...
    const migratedJournalEntries = (Array.isArray(state?.journalEntries) ? state.journalEntries : []).map((entry, index) => {
        ...
        return {
            ...
            type: typeof entry?.type === "string" && VALID_JOURNAL_TYPES.has(entry.type)
                ? entry.type
                : "note",
    ...
    const migratedWeeklyReviews = (Array.isArray(state?.weeklyReviews) ? state.weeklyReviews : []).map((review, index) => {
        const weekStart = typeof review?.weekStart === "string" && review.weekStart
            ? review.weekStart.slice(0, 10)
            : toIso(review?.createdAt ?? review?.closedAt, now).slice(0, 10);
        const weekEnd = typeof review?.weekEnd === "string" && review.weekEnd
        ...
        return {
            ...
            perProject: Array.isArray(review?.perProject)
                ? review.perProject.map((item, itemIndex) => {
            ...
            markdown: typeof review?.markdown === "string" && review.markdown
                ? review.markdown
    ...
    const migratedWeeklySnapshots = (Array.isArray(state?.weeklySnapshots) ? state.weeklySnapshots : []).map((snapshot, index) => {
        ...
        const weekEnd = typeof snapshot?.weekEnd === "string" && snapshot.weekEnd
    ...
    const migratedQuickCaptures = (Array.isArray(state?.quickCaptures) ? state.quickCaptures : []).map((capture, index) => ({
        id: typeof capture?.id === "string" && capture.id
            ? capture.id
            : fallbackId("quick-capture", index),
        type: typeof capture?.type === "string" && VALID_QUICK_CAPTURE_TYPES.has(capture.type)
            ? capture.type
            : "note",
    ...
    const migrated = {
        ...
        github: {
            ...
            user: typeof state?.github?.user?.login === "string" && state.github.user.login
                ? {
                    login: state.github.user.login,
                    avatarUrl: typeof state.github.user.avatarUrl === "string" ? state.github.user.avatarUrl : null,
                    name: typeof state.github.user.name === "string" ? state.github.user.name : null
                }
                : null,
            lastSyncAt: typeof state?.github?.lastSyncAt === "string" && state.github.lastSyncAt
...
export function applyMigrations(bundle) {
