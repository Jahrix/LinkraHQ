...
export function generateWeeklyReview(state: AppState, weekStart: string): WeeklyReview {
  ...
  const tasksCreated = state.projects.reduce((sum, project) => sum + project.tasks.length, 0);
  ...
  const focusMinutes = state.focusSessions.reduce((sum, session) => sum + session.durationMinutes, 0);
