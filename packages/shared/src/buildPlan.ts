...
export interface BuildPlanTaskContext {
  id: string;
  text: string;
  project: string;
  priority: string;
...
export interface BuildPlanPromptPayload {
  today: string;
  tasks: BuildPlanTaskContext[];
  systemPrompt: string;
  userMessage: string;
}
...
export function parseBuildPlanResponse(rawText: string, validTaskIds: string[]) {
  ...
  const filteredIds = parsed.taskIds
    .filter((taskId): taskId is string => typeof taskId === "string" && validIds.has(taskId))
    .slice(0, 6);
